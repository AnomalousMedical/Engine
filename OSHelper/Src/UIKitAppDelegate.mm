...
{
    if([uiKitViewController safeToRender])
    {
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [self.mFrameLink addToRunLoop:[NSRunLoop currentRunLoop] forMode:NSDefaultRunLoopMode];
    [uiKitViewController setApplicationActive:true];
    
    return YES;
}

#include "GLES2/gles2w.h"  //Questionable at best, should do this through ogre
...
- (void)applicationDidEnterBackground:(UIApplication *)application {
    ...
    [uiKitViewController setApplicationActive:false];
    glFinish();
}

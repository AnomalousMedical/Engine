...
extern "C" _AnomalousExport Ogre::Texture* TextureManager_createManual(String name, String group, Ogre::TextureType texType, uint width, uint height, uint depth, int num_mips, Ogre::PixelFormat format, Ogre::TextureUsage usage, bool hwGammaCorrection, uint fsaa, String fsaaHint, ProcessWrapperObjectDelegate processWrapper)
{
	try
	{
		const Ogre::TexturePtr& texturePtr = Ogre::TextureManager::getSingleton().createManual(name, group, texType, width, height, depth, num_mips, format, usage, 0, hwGammaCorrection, fsaa, fsaaHint);

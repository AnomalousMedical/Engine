...
ref class SoftBodyLinearJointDefinition : public BulletElementDefinition
{
private:
...
internal:
	...
	{
		return gcnew SoftBodyLinearJointDefinition(name);
	}

public:
	static SoftBodyLinearJointDefinition()
    {
        memberScanner->ProcessFields = false;
        memberScanner->Filter = gcnew EditableAttributeFilter();
    }

	SoftBodyLinearJointDefinition(System::String^ name);

	virtual ~SoftBodyLinearJointDefinition(void);
...
//Saving
protected:
	SoftBodyLinearJointDefinition(LoadInfo^ info);

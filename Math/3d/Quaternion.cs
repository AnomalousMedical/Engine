...
namespace Engine
{
    ...
    public struct Quaternion
    {
        ...
        /// <summary>
        /// Alternative shortest arc, experimental.
        /// </summary>
        /// <param name="src">The source vector.</param>
        /// <param name="dest">The destination vector.</param>
        /// <returns>The quaternion with the shortest arc.</returns>
        public static Quaternion computeShortestArc(ref Vector3 src, ref Vector3 dest)
        {
            Vector3 v0 = src;
            Vector3 v1 = dest;
            ...
            {
                // Generate an axis
                Vector3 axis = Vector3.UnitX.cross(ref src);
                if (axis.length() == 0)
                {
                    // pick another if colinear
                    axis = Vector3.UnitY.cross(ref src);
                }
                Vector3 normalAxis = axis.normalize();
                Quaternion q = new Quaternion(ref normalAxis, 3.1415926535897932f);
                return q;
            }
            else
            {
                ...
                Quaternion q = new Quaternion(c.x * inverse, c.y * inverse, c.z * inverse, s * 0.5f);
                return q.normalize();
            }
        }
        ...
        /// <returns></returns>
        public static Quaternion computeRotationForDirection(ref Vector3 targetDirection, ref Quaternion sourceOrientation)
        {
            targetDirection.normalize();

            //Compute current direction vector
            Vector3 currentDir = Quaternion.quatRotate(sourceOrientation, Vector3.Forward);

            Quaternion targetOrientation;
            //180 degree turn
            if ((currentDir + targetDirection).length2() < 0.00005f)
            {
                targetOrientation = new Quaternion(-sourceOrientation.z, sourceOrientation.w,
                                          sourceOrientation.x, -sourceOrientation.y);
            }
            else
            {
                // Derive shortest arc to new direction
                Quaternion rotQuat = Quaternion.computeShortestArc(ref currentDir, ref targetDirection);
        ...
        /// <param name="sourceOrientation"></param>
        /// <returns></returns>
        public static Quaternion computeRotationForLookAt(ref Vector3 lookAt, ref Vector3 origin, ref Quaternion sourceOrientation)
        {

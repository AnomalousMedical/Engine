...
namespace OgrePlugin
{
    class GlslesUnifiedShaderFactory : UnifiedShaderFactory
    {
        ...
        public GlslesUnifiedShaderFactory(ResourceManager liveResourceManager)
            : base(liveResourceManager)
        {
            var packProgram = HighLevelGpuProgramManager.Instance.createProgram("Pack_VP_GLSL", ResourceGroupName, "glsles", GpuProgramType.GPT_VERTEX_PROGRAM);
            packProgram.Value.SourceFile = UnifiedShaderBase + "Pack.glsl";
            packProgram.Value.load();
            localPrograms.Add(packProgram);

            var lightingProgram = HighLevelGpuProgramManager.Instance.createProgram("Lighting_FP_GLSL", ResourceGroupName, "glsles", GpuProgramType.GPT_FRAGMENT_PROGRAM);
            ...
            var unpackProgram = HighLevelGpuProgramManager.Instance.createProgram("Unpack_FP_GLSL", ResourceGroupName, "glsles", GpuProgramType.GPT_FRAGMENT_PROGRAM);
            unpackProgram.Value.SourceFile = UnifiedShaderBase + "Unpack.glsl";
            unpackProgram.Value.load();
            localPrograms.Add(unpackProgram);

            var texMipLevelProgram = HighLevelGpuProgramManager.Instance.createProgram("VirtualTextureFuncs_GLSL", ResourceGroupName, "glsles", GpuProgramType.GPT_FRAGMENT_PROGRAM);
